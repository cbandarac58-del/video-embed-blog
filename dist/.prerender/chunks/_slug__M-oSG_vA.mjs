...
import { S as unescapeHTML, T as createComponent, a as renderComponent, f as renderTemplate, g as maybeRenderHead, v as addAttribute, w as createAstro, y as defineScriptVars } from "./server_DdnRNRi_.mjs";
...
//#region src/pages/videos/[slug].astro
...
async function getStaticPaths() {
	return (await getCollection("videos")).map((video) => ({
...
var $$Slug = createComponent(async ($$result, $$props, $$slots) => {
	...
	function getSimilarityScore(videoA, videoB) {
		...
		for (const tag of tagsB) if (tagsA.has(tag)) intersectionSize++;
		let score = intersectionSize * 10;
		if (videoA.data.category === videoB.data.category) score += 5;
		return score;
	}
	const relatedVideos = allVideos.filter((v) => v.data.slug !== video.data.slug).map((v) => ({
		video: v,
		score: getSimilarityScore(video, v)
	})).sort((a, b) => b.score - a.score).slice(0, 4).map((item) => item.video);
	...
	return renderTemplate`${renderComponent($$result, "Layout", $$Layout, {
		...
		"isVideoPage": true
	}, { "default": async ($$result) => renderTemplate`<script type="application/ld+json">${unescapeHTML(JSON.stringify({
        ...
        })();<\/script><div id="country-block-banner" style="display:none;" class="mt-3 flex items-start gap-3 rounded-xl border border-amber-800/60 bg-amber-950/40 px-4 py-3 text-sm text-amber-200"><svg class="mt-0.5 h-5 w-5 shrink-0 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"></path></svg><div><p class="font-bold text-amber-300">This video may be restricted in your region.</p><p class="text-xs mt-0.5 text-amber-400">Please use a VPN to watch securely &mdash; turn on your VPN and reload the page.</p></div></div><!-- VPN Notice Bar --><div id="vpn-notice" class="mt-3 flex items-center justify-between gap-3 rounded-xl border border-slate-700/50 bg-slate-900/80 px-4 py-2.5 backdrop-blur-sm"><div class="flex items-center gap-2.5 text-xs text-slate-400"><svg class="h-4 w-4 shrink-0 text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg><span><strong class="text-slate-300">Video not loading?</strong>&nbsp;Please use a <strong class="text-rose-400">VPN</strong> to watch securely.</span></div><button id="vpn-notice-close" aria-label="Dismiss VPN notice" class="ml-2 shrink-0 text-slate-600 hover:text-slate-300 transition-colors"><svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div><!-- Video Title & Actions --><div class="mt-6 border-b border-slate-900 pb-6"><h1 class="text-xl font-extrabold text-white sm:text-2xl lg:text-3xl leading-snug">${uniqueTitle}</h1><!-- Stats Dashboard --><div class="flex flex-wrap items-center justify-between mt-4 gap-4 text-xs text-slate-400"><div class="flex items-center space-x-6"><!-- Views --><span class="flex items-center"><svg class="h-4 w-4 mr-1.5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg><strong class="text-slate-200 font-semibold mr-1">${formatViews(views)}</strong> Views</span><!-- Rating --><span class="flex items-center text-amber-400"><svg class="h-4 w-4 mr-1 text-amber-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"></path></svg><strong class="font-bold mr-1">${rating}%</strong> Rating</span><!-- Upload Date --><span class="hidden sm:inline">Added: <strong class="text-slate-300 font-semibold">${dateAdded}</strong></span></div><!-- Action buttons --><div class="flex items-center space-x-2"><button id="share-btn" class="flex items-center space-x-1.5 rounded-lg bg-slate-900 border border-slate-800 hover:border-rose-500 hover:text-white px-3 py-1.5 transition-colors"><svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 10.742s-1.839 2.51-1.839 5.258c0 1.543.826 2.37 2.148 2.37 1.44 0 2.215-.815 3.016-1.782.784.957 1.56 1.782 3.016 1.782 1.322 0 2.148-.827 2.148-2.37 0-2.748-1.839-5.258-1.839-5.258m-6.65 0L12 4l3.334 6.742"></path></svg><span id="share-btn-text">Share</span></button></div></div></div><!-- Tags / Categories --><div class="py-6"><h3 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">Tags & Categories</h3><div class="flex flex-wrap gap-2"><span class="text-xs rounded-lg bg-rose-950/60 border border-rose-800 text-rose-300 px-3 py-1 font-semibold uppercase">${category}</span>${tags.map((tag) => {
		return renderTemplate`<a${addAttribute(`/tag/${tag.toLowerCase().replace(/\s+/g, "-").replace(/[^\w\-]+/g, "").replace(/\-\-+/g, "-").replace(/^-+/, "").replace(/-+$/, "")}`, "href")} class="text-xs rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:border-slate-700 hover:text-slate-200 px-3 py-1 transition-colors">#${tag}</a>`;
	})}</div></div><!-- Related Videos Grid --><div class="mt-8 border-t border-slate-900 pt-8"><h2 class="text-lg font-extrabold text-white mb-6">Related Videos</h2>${relatedVideos.length === 0 ? renderTemplate`<p class="text-xs text-slate-500">No other videos in this category yet.</p>` : renderTemplate`<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">${relatedVideos.map((video) => renderTemplate`${renderComponent($$result, "VideoCard", $$VideoCard, { "video": video })}`)}</div>`}</div></div><!-- Sidebar Ads / Info (Righthand Column) --><div class="lg:col-span-4 flex flex-col space-y-6"><!-- Ad Banner (300x250 sidebar) -->${renderComponent($$result, "AdContainer", $$AdContainer, { "slotType": "banner_300x250" })}<!-- Compliance & Safe Content Info Box --><div class="rounded-xl border border-slate-900 bg-slate-950/80 p-5 text-slate-400 text-xs leading-relaxed space-y-3"><h3 class="font-bold text-slate-200 text-sm flex items-center space-x-1.5 border-b border-slate-900 pb-2"><span class="text-rose-500 font-extrabold">🔞</span><span>Safety &amp; Content Policy</span></h3><p>VixTube is a video embed directory indexing publicly hosted video links. We have zero tolerance for illegal content.</p></div><!-- Second Ad Banner in sidebar -->${renderComponent($$result, "AdContainer", $$AdContainer, { "slotType": "banner_468x60" })}</div></div><!-- Share Button + VPN Notice + Country Block Client Logic --><script>
    ...
    <\/script>${embedUrl.includes("xxxbp.tv") && renderTemplate`<script>(function(){${defineScriptVars({
		embedUrl,
		thumbnailUrl
	})}
...
var $$file = "C:/Users/Pubudu Nuwan/.gemini/antigravity/scratch/video-embed-blog/src/pages/videos/[slug].astro";
var $$url = "/videos/[slug]";
//#endregion
//#region \0virtual:astro:page:src/pages/videos/[slug]@_@astro
