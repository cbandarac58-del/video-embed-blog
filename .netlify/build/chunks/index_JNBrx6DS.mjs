...
import 'piccolore';
import { aZ as renderTemplate, a6 as addAttribute, aO as maybeRenderHead } from './params-and-props_NoTlu8e-.mjs';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (searchQuery) {
    ...
    filteredVideos = filteredVideos.filter(
      (v) => v.data.title.toLowerCase().includes(query) || v.data.category.toLowerCase().includes(query) || v.data.tags.some((t) => t.toLowerCase().includes(query))
    );
  }
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Watch Free Adult Videos, XXX Clips & Desi Porn" }, { "default": async ($$result2) => renderTemplate(_a || (_a = __template(["  ", '<section class="relative overflow-hidden bg-slate-950 py-12 border-b border-slate-900"> <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-rose-950/20 via-slate-950 to-slate-950"></div> <div class="relative mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8"> <h1 class="text-3xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl">\nWatch Free HD <span class="bg-gradient-to-r from-rose-500 via-pink-500 to-amber-500 bg-clip-text text-transparent">Porn & XXX Videos</span> </h1> <p class="mx-auto mt-4 max-w-2xl text-sm sm:text-base text-slate-400">\nDiscover the highest-rated amateur, desi bhabhi, stepsister, and hardcore streaming adult videos. Clean, responsive embeds optimized for all mobile devices.\n</p> </div> </section>  <section class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8"> <!-- Leaderboard Ad Banner --> ', ' <!-- Mobile Categories Horizontal Scroll --> <div class="mb-6"> <h2 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">Categories</h2> <div class="flex flex-wrap gap-2"> <a href="/"', ">\nAll Videos\n</a> ", ' </div> </div> <!-- Active Filter Indicators & Sorting --> <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between border-t border-slate-900 pt-6 mb-8 gap-4"> <!-- Filter status --> <div id="filter-status-text" class="text-sm text-slate-400"> ', ' <span class="text-xs text-slate-600 ml-1">(', ' items)</span> </div> <!-- Sorting Controls --> <div class="flex items-center space-x-2"> <span class="text-xs text-slate-500 font-medium">Sort By:</span> <div class="inline-flex rounded-lg bg-slate-900 p-0.5 border border-slate-800 text-xs"> <a href="/" class="rounded-md px-3 py-1 font-medium transition-colors bg-slate-800 text-white">\nRecent\n</a> <a href="/popular" class="rounded-md px-3 py-1 font-medium transition-colors text-slate-400 hover:text-slate-200">\nMost Viewed\n</a> <a href="/featured" class="rounded-md px-3 py-1 font-medium transition-colors text-slate-400 hover:text-slate-200">\nTop Rated\n</a> </div> </div> </div> <!-- Main Grid --> ', "  ", "  ", ' <!-- Footer Tag Cloud --> <div class="mt-16 border-t border-slate-900 pt-8"> <h3 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-4">Trending Tags</h3> <div class="flex flex-wrap gap-2"> ', ` </div> </div> <script>
  ...
  }), searchQuery ? renderTemplate`<span>Search results for "<span class="text-rose-400 font-medium">${searchQuery}</span>"</span>` : activeCategory ? renderTemplate`<span>Showing <span class="text-rose-400 font-medium">${activeCategory}</span> videos</span>` : activeTag ? renderTemplate`<span>Tagged with "<span class="text-rose-400 font-medium">${activeTag}</span>"</span>` : renderTemplate`<span>Showing <span class="text-rose-400 font-medium">All Trending</span> Videos</span>`, filteredVideos.length, filteredVideos.length === 0 ? renderTemplate`<div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-800 py-20 text-center"> <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-900 text-rose-500 border border-slate-800 mb-4 animate-bounce">
✦
</div> <h3 class="text-lg font-bold text-slate-200">No Videos Found</h3> <p class="text-xs text-slate-500 mt-1 max-w-sm">
Try adjusting your keywords, or add new video entries directly to the centralized database configuration.
</p> </div>` : renderTemplate`<div id="video-grid" class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">  ${pagedVideos.map((video) => renderTemplate`${renderComponent($$result2, "VideoCard", $$VideoCard, { "video": video })}`)} </div>`, totalPages > 1 && (() => {
    const currentPage = 1;
    const delta = 2;
    const pages = [];
    pages.push(1);
    ...
    return renderTemplate`<div id="pagination-container" class="mt-12 flex items-center justify-center gap-2 flex-wrap"> <span class="flex items-center gap-1 rounded-lg bg-slate-900/40 border border-slate-800/40 px-4 py-2 text-sm font-semibold text-slate-600 cursor-not-allowed">← Prev</span> ${pages.map(
...
Next →
  ...
  })(), filteredVideos.length === 0 && renderTemplate`<span></span>`, allTags.map((tag) => {
...
const $$file = "C:/Users/Pubudu Nuwan/.gemini/antigravity/scratch/video-embed-blog/src/pages/index.astro";
const $$url = "";
