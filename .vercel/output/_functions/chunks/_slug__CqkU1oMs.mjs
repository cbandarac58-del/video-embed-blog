...
import 'piccolore';
import { aX as maybeRenderHead, a6 as addAttribute, bb as renderTemplate, aq as defineScriptVars, bl as unescapeHTML } from './params-and-props_DsYBBh82.mjs';
import { r as renderComponent } from './entrypoint_Bjf-1lA6.mjs';
import { r as renderScript, a as getEntry, g as getCollection, $ as $$Layout } from './Layout_CEJTXjIt.mjs';
import 'clsx';
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  function getCardGradient(slug2) {
    const hash = slug2.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0);
    const gradients = [
...
&nbsp;Please use a <strong class="text-rose-400">VPN</strong> to watch securely.
  ...
  })), maybeRenderHead(), addAttribute(`/category/${catSlug}`, "href"), category, title, embedUrl.includes("xxxbp.tv") ? renderTemplate`<div id="xxxbp-player-container" class="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-slate-950"> <div class="flex flex-col items-center space-y-3"> <div class="animate-spin rounded-full h-10 w-10 border-b-2 border-rose-500"></div> <span class="text-xs text-slate-500">Loading premium ad-free player...</span> </div> </div>` : renderTemplate`<iframe${addAttribute(embedUrl, "src")} class="absolute top-0 left-0 w-full h-full" frameborder="0" allowfullscreen="true" scrolling="no"${addAttribute(title, "title")}></iframe>`, defineScriptVars({ embedUrl }), title, formatViews(views), rating, dateAdded, category, tags.map((tag) => {
...
#${tag} </a>`;
  }), relatedVideos.length === 0 ? renderTemplate`<p class="text-xs text-slate-500">No other videos in this category yet.</p>` : renderTemplate`<div class="grid grid-cols-1 sm:grid-cols-2 gap-6"> ${relatedVideos.map((video2) => renderTemplate`<a${addAttribute(`/videos/${video2.data.slug}`, "href")} class="group relative flex flex-col rounded-xl overflow-hidden border border-slate-900 bg-slate-950 hover:border-slate-800 hover:-translate-y-0.5 transition-all"> <div${addAttribute(`relative aspect-video w-full overflow-hidden flex items-center justify-center bg-slate-900 ${!video2.data.thumbnailUrl ? `bg-gradient-to-br ${getCardGradient(video2.data.slug)}` : ""}`, "class")}> ${video2.data.thumbnailUrl && renderTemplate`<img${addAttribute(video2.data.thumbnailUrl, "src")} referrerpolicy="no-referrer"${addAttribute(video2.data.title, "alt")} loading="lazy" class="absolute inset-0 h-full w-full object-cover group-hover:scale-105 transition-transform duration-500">`} <div class="absolute inset-0 bg-slate-950/40"></div> <div class="z-10 flex h-8 w-8 items-center justify-center rounded-full bg-slate-950/90 border border-slate-800 text-white group-hover:bg-rose-600 group-hover:border-rose-500 transition-all"> <svg class="h-3 w-3 ml-0.5" fill="currentColor" viewBox="0 0 24 24"> <path d="M8 5v14l11-7z"></path> </svg> </div> <span class="absolute bottom-2 right-2 rounded bg-slate-950/90 border border-slate-800/80 px-1.5 py-0.5 text-[9px] font-bold text-slate-400">
★ ${video2.data.rating}%
</span> </div> <div class="p-3"> <h3 class="text-xs font-bold text-slate-200 line-clamp-1 group-hover:text-white transition-colors"> ${video2.data.title} </h3> <div class="flex justify-between items-center text-[10px] text-slate-500 mt-2"> <span>${formatViews(video2.data.views)} views</span> <span>${video2.data.category}</span> </div> </div> </a>`)} </div>`, renderComponent($$result2, "AdContainer", $$AdContainer, { "slotType": "banner_300x250" }), renderComponent($$result2, "AdContainer", $$AdContainer, { "slotType": "banner_468x60" }), embedUrl.includes("xxxbp.tv") && renderTemplate(_a || (_a = __template(["<script>(function(){", `
